<?php 
   ...
   if(isset($_GET['id']) && $loggedPrivilege == 2){
      $id = $_GET['id'];
      $sql = mysqli_query($con, "SELECT * FROM donation WHERE Ot_id = '$id'");
      $get = mysqli_fetch_array($sql);

      $donationName = $get['Name'];
      $donationDate = $get['date'];
      $donationType = $get['type'];
      $donationAmt = $get['amount'];
   }else{
   ...
   if(isset($_POST['update'])){
      ...
      $type = $_POST['type'];
      $amount = $_POST['amount'];
      $query = mysqli_query($con, "UPDATE donation SET Name = '$name', date = '$date', type = '$type', amount = '$amount' WHERE Ot_id = '$id'");
      if($query) {
...
<head>
   ...
   <title>Update Donation - Kaine FC</title>
         ...
         <div class="addForm popUpform">
            ...
            <form method="POST" >
               <div class="form-inputs">
                  <label>Name</label>
                  <input type="text" name="name" required value="<?php echo $donationName?>">
               </div>
               <div class="form-inputs">
                  <label>Date</label>
                  <input type="date" name="date" min="<?php echo date('Y-m-d')?>" required value="<?php echo $donationDate?>">
               </div>
               <div class="form-inputs">
                  <label>Type</label>
                  <input type="text" name="type" required value="<?php echo $donationType?>">
               </div>
               <div class="form-inputs">
                  <label>Amount (RWF)</label>
                  <input type="number" name="amount" placeholder="Ex: 90900" required value="<?php echo $donationAmt?>">
               </div>
